...
public sealed class SrmSignatureEncoderTests {
	...
	public static void GenericSignatureWritesItsParameterCount() {
		IlMethodSignature signature = IlRefFactory.Signature(
			...
			parameterTypes: [IlRefFactory.MethodGenericParameter(1)],
	...
	public static void VoidPointerIsAccepted() =>
		Assert.Equal([0x0f, 0x01], typeSpec(IlRefFactory.Pointer(IlTest.Void)));
	...
	public static void GenericInstanceWritesDefinitionThenItsArguments() {
		...
		IlGenericInstanceTypeRef instance = IlRefFactory.GenericInstance(definition, IlTest.Int32);
	...
	public static void GenericParametersDistinguishTypeFromMethod() {
		Assert.Equal([0x13, 0x02], typeSpec(IlRefFactory.TypeGenericParameter(2)));
		Assert.Equal([0x1e, 0x02], typeSpec(IlRefFactory.MethodGenericParameter(2)));
	}
	...
	public static void SzArrayWrapsItsElementType() =>
		Assert.Equal([0x1d, 0x08], typeSpec(IlRefFactory.SzArray(IlTest.Int32)));

	[Fact]
	public static void MultidimensionalArrayWritesRankThenBounds() =>
		// ARRAY, I4, rank 2, 0 sizes, 0 lower bounds
		Assert.Equal([0x14, 0x08, 0x02, 0x00, 0x00], typeSpec(IlRefFactory.Array(IlTest.Int32, rank: 2)));

	[Fact]
	public static void ArraySizesAndLowerBoundsAreWrittenWhenPresent() {
		IlArrayTypeRef array = IlRefFactory.Array(IlTest.Int32, rank: 2, sizes: [3, 4], lowerBounds: [1, -1]);
	...
	public static void NestedPointerChainIsWrittenOutsideIn() =>
		Assert.Equal(
			[0x0f, 0x0f, 0x08],
			typeSpec(IlRefFactory.Pointer(IlRefFactory.Pointer(IlTest.Int32)))
		);
	...
	public static void ModifiersPrecedeTheTypeTheyModify() {
		...
		byte[] encoded = method(IlTest.Sig(IlRefFactory.RequiredModifier(modifier, IlTest.Int32)));
	...
	public static void OptionalModifiersUseTheirOwnElementType() {
		...
		byte[] encoded = method(IlTest.Sig(IlRefFactory.OptionalModifier(modifier, IlTest.Int32)));
	...
	public static void ModifiersPrecedeByRef() {
		...
		IlTypeRef parameter = IlRefFactory.RequiredModifier(modifier, IlRefFactory.ByRef(IlTest.Int32));
	...
	public static void ByRefIsRejectedInsideANestedType() =>
		...
		Assert.Throws<IlEncodingException>(
			() => typeSpec(new IlSzArrayTypeRef(IlRefFactory.ByRef(IlTest.Int32)))
		);

	[Fact]
	public static void ByRefIsAcceptedAtASlot() =>
		Assert.Equal([0x00, 0x01, 0x01, 0x10, 0x08], method(IlTest.Sig(IlTest.Void, IlRefFactory.ByRef(IlTest.Int32))));

	[Fact]
	public static void PinnedIsAcceptedOnlyInALocalSignature() {
		IlPinnedTypeRef pinned = new(IlRefFactory.ByRef(IlTest.Int32));
	...
	public static void FunctionPointerEmbedsItsSignature() =>
		...
		Assert.Equal(
			...
			typeSpec(IlRefFactory.FunctionPointer(IlTest.Sig(IlTest.Int32, IlTest.Int32)))
		);
	...
	public static void ArrayLowerBoundsUseCompressedSignedIntegers(int lowerBound, byte[] expected) {
		IlArrayTypeRef array = IlRefFactory.Array(IlTest.Int32, rank: 1, sizes: [], lowerBounds: [lowerBound]);
	...
	public static void GenericParameterIndicesUseCompressedUnsignedIntegers(int index, byte[] expected) =>
		Assert.Equal([(byte)0x1e, .. expected], typeSpec(IlRefFactory.MethodGenericParameter(index)));
