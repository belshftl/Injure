...
public sealed class StronglyTypedIntGenerator : IIncrementalGenerator {
	// ==========================================================================
	// internal types
	private sealed class TargetInfo(INamedTypeSymbol symbol, string? ns, ImmutableArray<string> containingTypeHeaders, string backingTypeName, bool isSigned) {
		public INamedTypeSymbol Symbol { get; } = symbol;
		public string? Namespace { get; } = ns;
		public ImmutableArray<string> ContainingTypeHeaders { get; } = containingTypeHeaders;
	...
	private static TargetInfo? check(GeneratorAttributeSyntaxContext ctx, CancellationToken ct) {
		...
		if (sym.TypeKind != TypeKind.Struct || !Util.Partial(sym, ct) || !sym.IsReadOnly ||
			sym.TypeParameters.Length != 0 || sym.IsRecord)
			return null;

		AttributeData attr = ctx.Attributes[0];
		ImmutableArray<string> containingTypeHeaders = ImmutableArray<string>.Empty;
		...
		if (sym.ContainingType is not null)
			containingTypeHeaders = Util.GetContainingTypeWrapperHeaders(sym).ToImmutableArray();

		return new TargetInfo(sym, Util.GetNamespace(sym), containingTypeHeaders, backingTypeName, isSigned);
	}

	private static int sizeofByName(string name) => name switch {
		"sbyte" or "byte" or "SByte" or "Byte" => 1,
		"short" or "ushort" or "Int16" or "UInt16" => 2,
		"int" or "uint" or "Int32" or "UInt32" => 4,
		"long" or "ulong" or "Int64" or "UInt64" => 8,
		"nint" or "nuint" or "IntPtr" or "UIntPtr" => IntPtr.Size,
		"Int128" or "UInt128" or "global::System.Int128" or "global::System.UInt128" => 16,
		_ => throw new ArgumentException($"unknown integer type '{name}'", nameof(name)),
	};
	...
	private static string emit(TargetInfo info) {
		...
		int backingSize = sizeofByName(backingType);
		...
		foreach (string header in info.ContainingTypeHeaders)
		...
		if (info.Namespace is not null)
			sb.Append("} // ").AppendLine(info.Namespace);
		return sb.ToString();
	}
