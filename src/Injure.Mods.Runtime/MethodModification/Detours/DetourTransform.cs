...
internal sealed class DetourTransform : IDetourTransform {
	...
	private static readonly IlMethodRef enterAndCheckRef = IlReferenceFactory.Method(
		typeof(DetourDispatch).GetMethod(nameof(DetourDispatch.EnterAndCheck), BindingFlags.Static | BindingFlags.Public)!
	);
	private static readonly IlMethodRef getChainEntryRef = IlReferenceFactory.Method(
	...
	/// <summary>
	/// Clears a method's chain.
	/// </summary>
	/// <remarks>
	/// Must happen before the method is reverted, not after, so that no new entry can reach the
	/// chain while the old body is still installed.
	/// </remarks>
	public void ClearChain(MethodIdentity method) {
		lock (slotsLock) {
