...
internal sealed class UntypedModExportTable : IStrongRefDroppable {
	...
	public UntypedModExportTable(Type lifetimeIdentityType) {
		...
		declsTyped = Activator.CreateInstance(typeof(ModExportDeclImpl<>).MakeGenericType(lifetimeIdentityType), this) ??
			throw new InternalStateException("Activator.CreateInstance instantiation of ModExportDeclImpl<> returned null");
	}
	...
	public void Add(Type contractType, object impl) {
		...
		InternalStateException.ThrowIfNull(contractType);
		...
		if (!contractType.IsInstanceOfType(impl))
			throw new InternalStateException($"contract impl is of type '{impl.GetType()}', expected '{contractType}'");
	...
	public ModExportDeclImpl<L> AsDeclsView<L>() where L : struct, IModLifetimeIdentity {
		...
		return (ModExportDeclImpl<L>)declsTyped;
	}

	private void validateContractType(Type contractType) {
		if (!contractType.IsInterface)
			throw new InternalStateException($"export contract '{contractType}' is not an interface");
		...
		if (!expectedMarker.IsAssignableFrom(contractType))
			throw new InternalStateException($"export contract '{contractType}' is not bounded over lifetime identity '{lifetimeIdentityType}'");
	}
...
internal sealed class ModExportDeclImpl<L> : IModExportDecl<L> where L : struct, IModLifetimeIdentity {
	private readonly UntypedModExportTable inner;

	public ModExportDeclImpl(UntypedModExportTable inner) {
