...
public sealed class ModRuntime<TGameApi> : IModuleOwnerResolver, IIlOwnerCtxProvider {
	...
	private readonly ModifRegistry modifRegistry = new();
	private readonly ModifOrchestrator modifOrchestrator;
	private readonly Dictionary<string, (DetourDeclSet Detours, PatchDeclSet Patches)> appliedModifsByOwner = new(StringComparer.Ordinal);
	private readonly Dictionary<string, ulong> nextGenerationByOwner = new(StringComparer.Ordinal);
	private LoadedCodeMod<TGameApi>[]? modifPassCode;
	private ResolvedModGraph modifPassGraph;
	...
	public async ValueTask StartAsync(CancellationToken ct = default) {
		...
		await DiscoverModifsAsync(ct).ConfigureAwait(false);
		await LoadAsync(ct).ConfigureAwait(false);
		await ApplyLoadModifsAsync(ct).ConfigureAwait(false);
	...
	public async ValueTask StageAsync(CancellationToken ct) {
		...
		try {
			...
			foreach (ResolvedMod mod in activeGraph.ModsInDeterministicOrder) {
				ct.ThrowIfCancellationRequested();
	...
	public ValueTask LoadContractAssembliesAsync(CancellationToken ct) {
		requirePhase(RuntimePhase.NativeLibrariesResolved, nameof(LoadContractAssembliesAsync));
		ct.ThrowIfCancellationRequested();
		try {
			Dictionary<string, string> pathsBySimpleName = new(StringComparer.OrdinalIgnoreCase);
			foreach (StagedMod mod in staged) {
				if (mod.Manifest is not CodeModManifest c)
					continue;
	...
	public async ValueTask LoadCodeAsync(CancellationToken ct) {
		requirePhase(RuntimePhase.ContractAssembliesLoaded, nameof(LoadCodeAsync));
		try {
			foreach (StagedMod mod in staged) {
				ct.ThrowIfCancellationRequested();
	...
	public async ValueTask DiscoverModifsAsync(CancellationToken ct) {
		requirePhase(RuntimePhase.CodeLoaded, nameof(DiscoverModifsAsync));
		ct.ThrowIfCancellationRequested();
		try {
			foreach (LoadedCodeMod<TGameApi> mod in activeCode.Values)
				ModifDiscoverer.DiscoverLoad(mod.Assembly, mod.LifetimeIdentityType, mod.LoadDetours, mod.LoadPatches);
			phase = RuntimePhase.ModifsDiscovered;
		} catch {
			phase = RuntimePhase.Faulted;
			throw;
		}
	...
	public async ValueTask LoadAsync(CancellationToken ct) {
		requirePhase(RuntimePhase.ModifsDiscovered, nameof(LoadAsync));
		ct.ThrowIfCancellationRequested();
		try {
	...
	public async ValueTask ApplyLoadModifsAsync(CancellationToken ct) {
		requirePhase(RuntimePhase.Loaded, nameof(ApplyLoadModifsAsync));
		ct.ThrowIfCancellationRequested();
		try {
			replaceModifLoad(activeCode.Values, activeGraph);
			phase = RuntimePhase.LoadModifsApplied;
		} catch {
			phase = RuntimePhase.Faulted;
			throw;
		}
	...
	public async ValueTask LinkAsync(CancellationToken ct) {
		requirePhase(RuntimePhase.LoadModifsApplied, nameof(LinkAsync));
		try {
	...
	public async ValueTask AbortAsync() {
		// TODO: decide whether to filter on `when (!ExceptionPolicy.IsInternalState(ex))`
		...
		try {
			...
			try {
				...
				diagnostics.Warning($"abort: error clearing modifs, moving on: {ex}");
			}
			...
			try {
				...
				diagnostics.Warning($"abort: error disposing module id lookup / modif orchestrator, moving on: {ex}");
			}
			try {
				prof.Dispose();
			} catch (Exception ex) {
	...
	bool IModuleOwnerResolver.TryGetOwner(ModuleId moduleId, [NotNullWhen(true)] out string? ownerId) {
		...
		if (gameAssemblies.Contains(asm)) {
			ownerId = gameOwnerId;
			return true;
		}
	...
	private static bool shutdownHasOwnerScopes(RuntimePhase phase) => phase.Tag switch {
		RuntimePhase.Case.CodeLoaded or
			RuntimePhase.Case.ModifsDiscovered or
			RuntimePhase.Case.Loaded or
			RuntimePhase.Case.LoadModifsApplied or
	...
	private async ValueTask processBoundaryAsync(ReloadBoundaryKind boundary, CancellationToken ct) {
		...
		try {
			for (;;) {
				...
				Transaction? txn = await prepareTransactionAsync(plan, ct).ConfigureAwait(false);
				if (txn is null)
					return;

				removePendingBatch(batch);

				ModOperationResult r = await commitTransactionAsync(txn, ct).ConfigureAwait(false);
				txn.DropContainersOnly();
				txn = null;
	...
	private void applyEnableToCandidateSet(string ownerId, EnableRequestKind kind, HashSet<string> candidateEnabled, HashSet<string> reloadRoots) {
		...
		bool enableRequiredDeps = kind.Tag is
		...
		HashSet<string> newlyEnabled = computeRequiredEnableClosure(ownerId, enableRequiredDeps, discovered);
	...
	private HashSet<string> computeRequiredEnableClosure(string rootOwnerId, bool enableRequiredDeps, Dictionary<string, DiscoveredMod> discoveredMap) {
		...
		try {
			foreach (StagedMod stagedMod in replacementStaged)
				if (stagedMod.Manifest is CodeModManifest) {
					...
					ModifDiscoverer.DiscoverLoad(loaded.Assembly, loaded.LifetimeIdentityType, loaded.LoadDetours, loaded.LoadPatches);
			...
			Dictionary<string, UntypedUntypedLoadedCodeDepInfo> candidateCodeOwnersInfo = buildCodeOwnerInfo(
				candidateCode
			);
	...
	private async ValueTask<ModOperationResult> commitTransactionAsync(Transaction txn, CancellationToken ct) {
		...
		try {
			...
			replaceModifLoad(
				activeCode.Values
					.Where(m => !plan.OldTouchedSet.Contains(m.Staged.Manifest.OwnerId))
					.Concat(txn.PreparedCode.Values),
				txn.CandidateGraph
			);
		...
		try {
			foreach (LoadedCodeMod<TGameApi> old in txn.OldCode.Values)
		...
		txn.DropPreparedStrongReferences();
		return r;
	}
	...
	private void replaceModifLoad(IEnumerable<LoadedCodeMod<TGameApi>> mods, ResolvedModGraph graph) {
		LoadedCodeMod<TGameApi>[] set = mods.ToArray();
		modifPassCode = set;
		modifPassGraph = graph;
		try {
			HashSet<string> seen = new(StringComparer.Ordinal);
			foreach (LoadedCodeMod<TGameApi> mod in set) {
				seen.Add(mod.Staged.Manifest.OwnerId);
				applyModifOwner(mod);
			}
			foreach (string ownerId in appliedModifsByOwner.Keys.Where(id => !seen.Contains(id)).ToArray()) {
				modifRegistry.RemoveOwner(ownerId);
				appliedModifsByOwner.Remove(ownerId);
			}
			modifOrchestrator.ApplyPending();
		} finally {
			modifPassCode = null;
			modifPassGraph = default;
		}
	}

	private void applyModifOwner(LoadedCodeMod<TGameApi> mod) {
		string ownerId = mod.Staged.Manifest.OwnerId;
		if (
			appliedModifsByOwner.TryGetValue(ownerId, out (DetourDeclSet Detours, PatchDeclSet Patches) current) &&
			current.Detours == mod.LoadDetours && current.Patches == mod.LoadPatches
		)
			return;

		modifRegistry.RemoveOwner(ownerId);

		foreach (PatchDeclDesc decl in mod.LoadPatches.Declarations) {
			MethodIdentity target = resolveModifIdentity(decl.Target);
			modifRegistry.AddManipulator(target, new OwnerOrderedEntry<IlManipulatorRegistration>(
				decl.Registration,
				ownerId,
				decl.Registration.LocalId,
				decl.LocalOrder,
				decl.Before,
				decl.After
			));
		}
		foreach (DetourDeclDesc decl in mod.LoadDetours.Declarations) {
			MethodIdentity target = resolveModifIdentity(decl.Target);
			modifRegistry.AddDetour(target, new OwnerOrderedEntry<DetourRegistration>(
				decl.Registration,
				ownerId,
				decl.Registration.LocalId,
				decl.LocalOrder,
			...
			modifOrchestrator.SyncDetourChain(target);
		}

		appliedModifsByOwner[ownerId] = (mod.LoadDetours, mod.LoadPatches);
	}

	private MethodIdentity resolveModifIdentity(MethodBase target) {
		if (!moduleIdLookup.TryGetModuleId(target.Module, out ModuleId moduleId))
			throw new InternalStateException($"target method '{target}' is declared in module '{target.Module}', which the profiler doesn't know about");
		return new MethodIdentity(moduleId, target.MetadataToken);
	}

	private MethodBase resolveModifMethod(MethodIdentity identity) {
		if (!moduleIdLookup.TryGetModule(identity.Module, out Module? module))
			throw new InternalStateException($"modif registry holds a MethodIdentity for module '{identity.Module}', which the profiler doesn't know about");
		return module.ResolveMethod(identity.MethodDefToken)!;
	}

	private void clearModifs() {
		foreach (string ownerId in appliedModifsByOwner.Keys.ToArray())
			modifRegistry.RemoveOwner(ownerId);
		appliedModifsByOwner.Clear();
		modifOrchestrator.ApplyPending();
	}
	...
	private LoadedCodeMod<TGameApi> loadCodeMod(StagedMod stagedMod) {
		...
		try {
			...
			scope = new UntypedBoundedScopeImpl(stagedMod.Generation, maxScopeTeardownParallelism);
			Type entryType = ModTypeDiscovery.FindEntrypointType(assembly, typeof(TGameApi), lifetimeIdentityType, stagedMod.EntryAssemblyPath);
			object entrypoint = Activator.CreateInstance(entryType)!;
			object? reloadEntrypoint = null;
			if (manifest.LiveReloadable) {
				Type reloadEntrypointType = ModTypeDiscovery.FindReloadEntrypointType(assembly, typeof(TGameApi), lifetimeIdentityType, stagedMod.EntryAssemblyPath);
	...
	private async ValueTask runLoadAsync(LoadedCodeMod<TGameApi> mod, CancellationToken ct) {
		object ctx = createLoadContext(mod, createApi(mod), diagnosticsSinkRegistry);
		try {
	...
	private async ValueTask runLinkAsync(
		...
		IReadOnlyDictionary<string, UntypedLoadedDepInfo> owners,
		IReadOnlyDictionary<string, UntypedUntypedLoadedCodeDepInfo> codeOwners,
		CancellationToken ct
	) {
		Dictionary<string, UntypedLoadedDepInfo>? deps = buildDeclaredDepInfo(mod.Staged.Manifest, owners);
		Dictionary<string, UntypedUntypedLoadedCodeDepInfo>? codeDeps = buildDeclaredDepInfo(mod.Staged.Manifest, codeOwners);
		object ctx = createLinkContext(mod, createApi(mod), diagnosticsSinkRegistry, deps, codeDeps);
		try {
	...
	private void publishTransaction(Transaction txn) {
		BoundaryPlan plan = txn.Plan;
		discovered = txn.CandidateDiscovered;
		activeGraph = txn.CandidateGraph;
		...
		foreach (KeyValuePair<string, LoadedCodeMod<TGameApi>> kvp in txn.PreparedCode)
			activeCode[kvp.Key] = kvp.Value;
		foreach (KeyValuePair<string, LoadedContentMod> kvp in txn.PreparedContent)
			activeContent[kvp.Key] = kvp.Value;
		staged = staged.Where(mod => !plan.OldTouchedSet.Contains(mod.Manifest.OwnerId)).Concat(txn.ReplacementStaged).ToArray();
	}
	...
	private static object createLoadPatchDecl(LoadedCodeMod<TGameApi> mod) => Activator.CreateInstance(
		typeof(PatchDeclImpl<>).MakeGenericType(mod.LifetimeIdentityType),
		mod.LoadPatches
	)!;
	...
	private static object createLoadContext(
		...
		typeof(ModLoadCtxImpl<,>).MakeGenericType(typeof(TGameApi), mod.LifetimeIdentityType),
		createLoadDetourDecl(mod),
		createLoadPatchDecl(mod),
		mod.Exports,
	...
	private static object createLinkContext(
		...
		TGameApi api,
		DiagnosticsSinkRegistry diagnosticsSinkRegistry,
		IReadOnlyDictionary<string, UntypedLoadedDepInfo> deps,
		IReadOnlyDictionary<string, UntypedUntypedLoadedCodeDepInfo> codeDeps
	) => Activator.CreateInstance(
		typeof(ModLinkCtxImpl<,>).MakeGenericType(typeof(TGameApi), mod.LifetimeIdentityType),
		deps,
		codeDeps,
		mod.Staged.Manifest.OwnerId,
		mod.Staged.Manifest.Version,
	...
	private static Dictionary<string, TInfo> buildDeclaredDepInfo<TInfo>(ModManifest manifest, IReadOnlyDictionary<string, TInfo> loaded) where TInfo : struct {
	...
	private PendingAlcUnload detachForUnload(LoadedCodeMod<TGameApi> mod) {
		...
		if (moduleIdLookup.TryGetModuleId(mod.Assembly.ManifestModule, out ModuleId moduleId))
			modifOrchestrator.ForgetModule(moduleId);
		moduleIdLookup.ForgetAssembly(mod.Assembly);
