...
public sealed class IlMatchException : IlPipelineException {
	...
	internal static IlMatchException ExpectedAny(in IlPatternSearchInfo info) => new(
		...
		{IlPatternDisplay.FormatPattern(in info)}
	...
	internal static IlMatchException ExpectedOne(in IlPatternSearchInfo info, int matchedCount) => new(
		...
		{IlPatternDisplay.FormatPattern(in info)}
		searching {info.Direction.Display} from instruction boundary {info.StartInstructionBoundary.ToString(CultureInfo.InvariantCulture)}, matched {matchedCount.ToString(CultureInfo.InvariantCulture)} occurrences
		"""
	);
}
