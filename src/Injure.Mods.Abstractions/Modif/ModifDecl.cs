...
public interface IDetourDecl<L> where L : struct, IModLifetimeIdentity {
	void Declare(Type generatedTargetType, MethodInfo impl, in DetourConfig config);
...
public interface IPatchDecl<L> where L : struct, IModLifetimeIdentity {
	void Declare(Type generatedTargetType, IlManipulator<L> manipulator, in PatchConfig config);
