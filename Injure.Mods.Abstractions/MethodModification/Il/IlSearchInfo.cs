	...
	public static string FormatElement(IlPatternElement element) {
		return element.Kind switch {
			...
			IlPatternElementKind.CecilField =>
				$"{element.OpCode.Name} {CecilDisplay.FormatField(element.CecilField ?? throw new InternalStateException("IlPatternElement with kind CecilField is missing its CecilField value"))}",
			IlPatternElementKind.ReflectionField =>
				$"{element.OpCode.Name} {ReflectionDisplay.FormatField(element.ReflectionField ?? throw new InternalStateException("IlPatternElement with kind ReflectionField is missing its ReflectionField value"))}",

			IlPatternElementKind.CecilMethod =>
				$"{element.OpCode.Name} {CecilDisplay.FormatMethod(element.CecilMethod ?? throw new InternalStateException("IlPatternElement with kind CecilMethod is missing its CecilMethod value"))}",
			IlPatternElementKind.ReflectionMethod =>
				$"{element.OpCode.Name} {ReflectionDisplay.FormatMethod(element.ReflectionMethod ?? throw new InternalStateException("IlPatternElement with kind ReflectionMethod is missing its ReflectionMethod value"))}",
	...
	public static string FormatProvenance(IlPatternProvenanceConstraint provenance) {
		return provenance.Kind switch {
			...
			IlPatternProvenanceConstraint.ConstraintKind.AllFromOwner =>
				$"provenance owner '{provenance.OwnerId ?? throw new InternalStateException("IlPatternProvenanceConstraint with kind AllFromOwner is missing its OwnerId value")}'",
