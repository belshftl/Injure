	...
	private void appendManagedDelegate(List<IlOutputNode> output, IlInstructionSpec spec) {
		if (managedDelegateLowerer is null)
			throw new InternalStateException(
				$"IL manipulator '{currProvenance.OwnerId}::{currProvenance.LocalId}' emitted a managed delegate but no managed-delegate lowerer was configured"
			);
	...
	private bool matchesElement(Instruction instr, IlPatternElement elem) {
		return elem.Kind switch {
			...
			IlPatternElementKind.ReflectionField =>
				instr.OpCode.Code == elem.OpCode.Code && instr.Operand is FieldReference f &&
				CecilMemberIdentity.SameField(
					f,
					elem.ReflectionField ?? throw new InternalStateException("IlPatternElement of kind ReflectionField is missing its ReflectionField value")
				),

			IlPatternElementKind.CecilMethod =>
				instr.OpCode.Code == elem.OpCode.Code && instr.Operand is MethodReference m &&
				CecilMemberIdentity.SameMethod(
					m,
					elem.CecilMethod ?? throw new InternalStateException("IlPatternElement of kind CecilMethod is missing its CecilMethod value")
				),
			...
			IlPatternElementKind.ReflectionMethod =>
				...
				CecilMemberIdentity.SameMethod(
					m,
					elem.ReflectionMethod ?? throw new InternalStateException("IlPatternElement of kind ReflectionMethod is missing its ReflectionMethod value")
				),
		...
		for (int i = 0; i < pattern.Length; i++)
			if (!pattern[i].IsValid)
				throw new ArgumentException(
					$"IL pattern element at index {i} is invalid; use MatchIl.Any if you intended to do a single-instruction wildcard match",
					paramName
				);
	}
